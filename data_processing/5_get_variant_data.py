...
if __name__ == "__main__":
    ...
    parser.add_argument("--disable_baq", action="store_true", help="whether to disable BAQ computation in read2vcf_cdna.sh")
    ...
    if not os.path.exists(config.index) or not os.path.exists(config.t2g):
        if not os.path.exists(config.preliminary_variants_vcf):
            ...
            if isinstance(config.fastqs, list):
                ...
                find_cmd = [
                    "bash", "-c",
                    f"find {fastq_dir} -type f | grep -E '{'_2\\.(fq|fastq)(\\.gz)?$'}' || true"
                ]
